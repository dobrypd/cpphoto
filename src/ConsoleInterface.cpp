...
void ConsoleInterface::directoryFromArgs(const char * fromDIR) throw(IOWhileLoadListOfFiles)
{
	directoryFromArgs(fromDIR, 0);
}

void ConsoleInterface::directoryFromArgs(const char * fromDIR, const char * toDIR) throw(IOWhileLoadListOfFiles)
{
	this->sourceDIR =
			(fromDIR == 0) ?
					(fs::current_path()):
					(fs::path(fromDIR, fs::native));
	this->destinationDIR =
			(toDIR == 0) ?
